...
To study this, fix $v \in T_p M$ and let $\gamma(t) = \exp_p(vt)$. For a smooth curve $w \colon (-\epsilon, \epsilon) \to T_p M$ with $w(0) = v$, the map
\[
\phi(s, t) = \exp_p(w(s) \cdot t)
\]

\todo[inline]{ exercise 6.3? }

defines a variation of $\gamma$ in which each $s$-curve $t \mapsto \phi(s, t)$ is a geodesic from $p$ with initial velocity $w(s)$. Taking the linear family $w(s) = v + s \, w_0$ for $w_0 \in T_p M$, the variation vector field at $t = 1$ is
\[
...
\begin{proof}
For all $w \in T_{tv}(T_p M)$: if $\phi(t, s) = \exp_p(t \cdot (v + sw))$, then
\[
