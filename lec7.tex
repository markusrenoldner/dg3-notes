...
Since $\Gamma_{ij}^k(0)$ is symmetric in $i, j$, we obtain $\Gamma_{ij}^k(0) = 0$.
...
\begin{tikzpicture}[scale=1]
    ...
    \draw[Leman!100, dashed] (0,0) circle (0.5);
    \draw[Leman!100, dashed] (0,0) circle (1);
    \draw[Leman!100, dashed] (0,0) circle (1.5);
\end{tikzpicture}
\end{center}
...
So of course this lemma was just an application of the Gauss lemma rephrased in polar coordinates. but we see that in polar coordinates, the theorem is particularly useful, because the radial direction is special in this setting.\\
...
\begin{equation}\label{eq:radial_length_bound}
\ell(\gamma|_{[0,t_0]}) = \int_0^{t_0} \sqrt{\dot r^2 + b^2 \dot\theta^2}\,\mathrm{d}t \geqslant \int_0^{t_0} |\dot r|\,\mathrm{d}t \geqslant r(t_0) - r(0) = r(t_0).
\end{equation}
Since $\ell(\gamma) = d < i(p)$, the curve cannot escape the chart: if it did, $r(t)$ would reach $i(p)$, forcing $\ell(\gamma) \geqslant i(p)$, a contradiction. So $\gamma([0,1])$ lies in the chart, and applying \eqref{eq:radial_length_bound} at $t_0 = 1$ gives $d = \ell(\gamma) \geqslant r(1)$. Conversely, the radial geodesic from $p$ to $\gamma(1)$ has length $r(1)$, so $d \leqslant r(1)$. Hence $\ell(\gamma) = r(1)$, i.e. equality holds in \eqref{eq:radial_length_bound}.

In particular, at every $t$ where $\gamma$ is differentiable,
\[
\sqrt{\dot r^2 + b^2(r,\theta)\dot\theta^2} = |\dot r|,
\]
so $b(r,\theta)\dot\theta = 0$. Since $b > 0$ for $r > 0$, we get $\dot\theta(t) = 0$ for $t > 0$. Thus $\theta$ is constant along $\gamma$, so $\gamma$ is a radial curve. Reparametrizing by arc length (i.e. parametrise so that $\left\lVert \dot\gamma(t) \right\rVert$ is constant in $t$) gives a radial geodesic from $p$.
\end{proof}
...
\begin{colordef}{Locally length-minimizing curve}{locally_length_minimizing}
...
We saw earlier the variational fact that geodesics are critical points of the length functional (Lemma \ref{lem:geodesics_critical_points_length}). The corollary below upgrades this to a genuine \emph{minimization} statement, by applying Theorem~\ref{thm:radial_geodesics_minimize} locally; at each point one identifies distance-realizing curves as radial geodesics from that point.
