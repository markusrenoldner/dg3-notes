...
\end{enumerate}
\end{colordef}
We will sometimes use the notation $\langle v, w\rangle := g(v, w)$ for the metric.
...
Let $M=\mathbb{R}^n$, then we define the Euclidean metric (or \emph{flat} metric) $g_E$ (or $g_{\mathbb{R}^n}$) to coincide with the Euclidean inner product on $\mathbb{R}^n$:
\[
...
\end{colordef}
More concretely: objects in $M$ or $T_pM$ (or products and sections thereof) that can be described in different coordinates may have such properties.
...
\end{enumerate}
\end{colorlem}
Property 3 shows that the length is a geometric quantity, independent of the parametrization.
\begin{proof}
See exercise 2.1.
\end{proof}
...
\label{subsec:integration-riemannian}
...
The wedge-product integral on the right-hand side is the usual integral on $\mathbb{R}^n$, i.e.\ 
$$\int_{\phi(U)} f\, \mathrm{d}x^1 \wedge \cdots \wedge \mathrm{d}x^n := \int_{\phi(U)} f\, \mathrm{d}x^1 \cdots \mathrm{d}x^n,$$
...
\subsection{Isometries and musical isomorphisms}
