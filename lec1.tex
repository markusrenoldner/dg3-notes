...
We have that $f^* : C^k(N) \to C^k(M)$ is an $\mathbb{R}$-linear map.
\end{colordef}
...
\]
The $\mathbb{R}$-vector space of tangent vectors at p is then denoted by $T_pM$. It has dimension $n$.
\end{colordef}

There is a natural basis of $T_pM$ given by the tangent vectors along the coordinate curves. 


\begin{colordef}{Coordinate basis vectors}{coordinate_basis}
Let $M,n,p,f,g,v$ be as above.\\
We define the coordinate basis vector
$$\left.\frac{\mathrm{d}}{\mathrm{d}x^i}\right|_p f := \left(\frac{\mathrm{d}}{\mathrm{d}x^i} (f \circ \varphi)\right) \bigg|_{\varphi(p)}.$$
\end{colordef}

\begin{colorlem}{Basis of $T_pM$}{basis_tangent_space}
The set $\left\{\left.\frac{\mathrm{d}}{\mathrm{d}x^i}\right|_p\right\}_{i=1}^n$ is a basis of $T_pM$. 
\end{colorlem}
\begin{proof}
...
In local coordinates: $\{\mathrm{d}x^i\}_{i=1}^n$ is the dual basis of $\left\{\frac{\mathrm{d}}{\mathrm{d} x^i}\right\}_{i=1}^n$, as we have
\[
\mathrm{d}x^i \left( \frac{\mathrm{d}}{\mathrm{d} x^j} \right) = \frac{\mathrm{d}}{\mathrm{d} x^j} (x^i) = \delta_j^i.
\]
...
\[v = v^i \frac{\mathrm{d}}{\mathrm{d} x^i}
\]instead of
\[v = \sum_{i=1}^n v^i \frac{\mathrm{d}}{\mathrm{d} x^i}
\]
...
\]
\[
\frac{\mathrm{d}}{\mathrm{d} x^i} = \frac{\partial y^j}{\partial x^i} \frac{\mathrm{d}}{\mathrm{d} y^j}
\]
\end{colorlem}
\begin{proof}
See chapter 3 in \cite{tsakanikas2024dg2}.
...
$$v=v^i \frac{\mathrm{d}}{\mathrm{d}x^i}=\tilde v^j \frac{\mathrm{d}}{\mathrm{d}y^j}$$
...
$$\omega_i = \omega\left(\frac{\mathrm{d}}{\mathrm{d}x^i}\right)$$ 
\end{colremark}
...
In local coordinates this is the jacobian matrix of $F$:
\[
[\mathrm{d}F]^a_b = \frac{\partial F^a}{\partial x^b}
\]

If $\gamma:(-\epsilon,\epsilon)\to M$ is a curve with $\gamma(0)=p$ and $\gamma'(0)=v$, then
\[
\mathrm{d}F_p(\gamma'(0)) = (F\circ \gamma)'(0) = \frac{\mathrm{d}}{\mathrm{d}t} (F\circ \gamma(t))\bigg|_{t=0}
\]
\end{colordef}
