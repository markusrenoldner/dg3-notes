...
\[
X:t \longmapsto X(t) \in T_{\gamma(t)} M
\]
a \emph{vector field along a curve} for $t \in I$, assigning to each $t$ a tangent vector at $\gamma(t)$. 
\end{colordef}

A vector field along a curve can equivalently be seen as a section $t\mapsto(t,X(t))$ of the bundle $\gamma^* TM$, the latter being the pullback via $\gamma$ of the tangent bundle
$$\gamma^* TM = \{(t,v) \in I \times TM : v \in T_{\gamma(t)} M\}.$$

Each fiber $\pi^{-1}(t) = {t}\times T_{\gamma(t)}M \cong T_{\gamma(t)}M$
% Each fiber $\pi^{-1}(t)= T_{\gamma(t)} M$ 
of $\gamma^* TM$ is a vector space, where the bundle projection is given by $\pi: \gamma^* TM \to I, (t,v) \mapsto t$.

Hence we can denote the space of all vector fields along $\gamma$ by $\Gamma(\gamma^* TM)$.

\begin{colremark}
If $\gamma$ self-intersects (i.e., $\gamma(t_1) = \gamma(t_2)$), $X(t_1)$ does not have to be the same as $X(t_2)$ (so not a unique value at all points of $M$ belonging to the image of $\gamma$).
...
& = \Gamma_{ij}^a g_{ah} + \Gamma_{ih}^a g_{aj} \label{eq:christoffel_perm_i}
\end{align}

Permuting indices:
\begin{align}
\frac{\partial}{\partial x^h} g_{ij} &= \Gamma_{hj}^a g_{ai} + \Gamma_{hi}^a g_{aj} \label{eq:christoffel_perm_h} \\
\frac{\partial}{\partial x^j} g_{ih} &= \Gamma_{ji}^a g_{ah} + \Gamma_{jh}^a g_{ai} \label{eq:christoffel_perm_j}
\end{align}

So \eqref{eq:christoffel_perm_h} + \eqref{eq:christoffel_perm_j} - \eqref{eq:christoffel_perm_i}:
\[
...
\begin{align}
X \langle Y, Z \rangle &= \langle \nabla_X Y, Z \rangle + \langle Y, \nabla_X Z \rangle \label{eq:koszul_deriv_1} \\
Y \langle X, Z \rangle &= \langle \nabla_Y X, Z \rangle + \langle X, \nabla_Y Z \rangle \label{eq:koszul_deriv_2} \\
Z \langle X, Y \rangle &= \langle \nabla_Z X, Y \rangle + \langle X, \nabla_Z Y \rangle \label{eq:koszul_deriv_3}
\end{align}

We use now the torsion-free condition, $\nabla_X Y - \nabla_Y X = [X, Y]$. From \eqref{eq:koszul_deriv_1}, \eqref{eq:koszul_deriv_2}, \eqref{eq:koszul_deriv_3}, we obtain Koszul's formula:
\[
\langle \nabla_X Y, Z \rangle
= \frac{1}{2} \Big(
...
In short: an isometry leaves every term of Koszul's formula invariant, so it must map the Levi-Civita connection of $g$ to that of $h$.\\

\begin{proof}[Proof sketch]
Every term in Koszul's formula is respected by $dF$. Since $F$ is a (local) isometry it preserves the inner product,
\[
\langle dF(X), dF(Y) \rangle_h = \langle X, Y \rangle_g ,
\]
and, being a local diffeomorphism, its pushforward preserves the Lie bracket,
\[
[dF(X), dF(Y)] = dF([X, Y]) .
\]
Hence Koszul's formula for $\nabla^{(h)}$, evaluated on $dF(X), dF(Y), dF(Z)$, reduces term by term to Koszul's formula for $\nabla^{(g)}$ on $X, Y, Z$, so that
\[
\langle \nabla^{(h)}_{dF(X)} dF(Y),\, dF(Z) \rangle_h = \langle dF(\nabla^{(g)}_X Y),\, dF(Z) \rangle_h
\qquad \text{for all } Z .
\]
Since $dF$ is a linear isomorphism on each tangent space, $dF(Z)$ ranges over all tangent vectors; as $h$ is nondegenerate, the two sides agree as vectors,
\[
dF(\nabla^{(g)}_X Y) = \nabla^{(h)}_{dF(X)} dF(Y) . \qedhere
\]
\end{proof}
...
Let $\gamma: I \to M$ be smooth and $\nabla$ any connection. For $s, t \in I$, the \emph{parallel transport} along $\gamma$ is the map
\[
P_{s,t} \colon T_{\gamma(s)}M \longrightarrow T_{\gamma(t)}M, \qquad P_{s,t}(v) := X(t),
\]
where $X$ is the unique \emph{parallel vector field along} $\gamma$, namely $\nabla_{\dot\gamma} X = 0$, and satisfies $X(s) = v$.
\end{colordef}

With this terminology, $X(t) = P_{s,t}(X(s))$ for every parallel field $X$ along $\gamma$.

\begin{colortheo}{Existence and Uniqueness of Parallel Transport}{}
For every $p \in M$, $v \in T_pM$, and smooth curve $\gamma: I \to M$ with $\gamma(0) = p$, there exists a unique vector field $X$ along $\gamma$ such that $X(0) = v$ and $\nabla_{\dot{\gamma}} X = 0$.
...
Concerning the isometry property, let $X, Y$ be vector fields on $\gamma$ s.t. $\nabla_{\dot\gamma} X = \nabla_{\dot\gamma} Y = 0$. Then
$$\frac{\mathrm{d}}{\mathrm{d}t} \left \langle X,Y \right \rangle = \dot\gamma (\left \langle X,Y \right \rangle) = \left \langle \nabla_{\dot\gamma} X,Y \right \rangle + \left \langle X,\nabla_{\dot\gamma} Y \right \rangle = 0,$$

where we have used that for $f(p):=\left \langle X_p,Y_p \right \rangle$ we have $\frac{\mathrm{d}}{\mathrm{d}t}(f\circ \gamma)(t) = \mathrm{d}\gamma \left ( \frac{\mathrm{d}}{\mathrm{d}t} \right )(f)=\dot\gamma(t)f$. 
...
\begin{colremark}
If we reparametrize: $\tilde\gamma = \gamma \circ h$: one can show that $\nabla_{\dot{\tilde\gamma}} \dot{\tilde\gamma} \parallel \dot{\tilde\gamma}$, i.e. there exists a function $\lambda(t)$ such that $\nabla_{\dot{\tilde\gamma}} \dot{\tilde\gamma} =\lambda \dot{\tilde\gamma}$. But we do not necessarily have $\nabla_{\dot{\tilde\gamma}} \dot{\tilde\gamma}=0$ anymore so the above definition is parametrisation dependent. Any curve which satisfies $\nabla_{\dot{\tilde\gamma}} \dot{\tilde\gamma} =\lambda \dot{\tilde\gamma}$ can always be reparametrised to geodesics.
\end{colremark}
...
\begin{tikzpicture}[scale=2]
    ...
    % Draw tangent vector
    \draw[->, thick, Leman, shift={(0,1)}] (0,0) -- (1,0);
    \node[right, above, Leman] at (1,1) {$\dot\gamma$};
    ...
    % acceleration
    \draw[->, thick, Leman, shift={(0,0)}] (0,1) -- (0,0.5);
    \node[right, Leman] at (0,0.5) {$\ddot\gamma$};
...
On a general Riemannian manifold, this is not necessarily true (for example, on the sphere). However, geodesics are always stationary points of the length functional (Lemma~\ref{lem:geodesics_critical_points_length}), and they still minimize length \emph{locally}, i.e.\ between sufficiently close points (Corollary~\ref{cor:geodesics_minimise_length_locally}).
\end{colremark}

From the definition, we can easily show that $\left\lVert \dot\gamma \right\rVert^2=g(\dot\gamma,\dot\gamma)$ is constant in $t$, i.e. geodesics are parametrised at constant speed.
...
$$\frac{\mathrm{d}}{\mathrm{d}t} g(\dot\gamma,\dot\gamma)=0.$$
\end{colorlem}
\begin{proof}
    \begin{align*}
        \frac{\mathrm{d}}{\mathrm{d}t} g(\dot\gamma,\dot\gamma)
        &= \mathrm{d} \gamma \left ( \frac{\mathrm{d}}{\mathrm{d}t} \right ) \left ( g(\dot\gamma,\dot\gamma) \right )\\
        &= \dot\gamma(t) g(\dot\gamma,\dot\gamma)\\
        &= g(\nabla_{\dot\gamma}\dot\gamma,\dot\gamma) + g(\dot\gamma,\nabla_{\dot\gamma}\dot\gamma). 
    \end{align*}
    But $\nabla_{\dot\gamma}\dot\gamma=0$ as $\gamma$ is a geodesic, so we yield the result.
\end{proof}
...
\begin{colexample}{Poincaré Half-Plane, geodesics and constants of motion}{}
...
\begin{tikzpicture}[scale=1.0,>=stealth]
    ...
    \draw[->,very thick,Leman] (0.0,0) arc[start angle=180,end angle=0,radius=2.0];
