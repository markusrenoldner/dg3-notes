...
Recall $R(X, Y, Z, W) = \langle R(X, Y) W,\, Z\rangle$ (Definition~\ref{def:04-riemann-curvature-tensor}). We expand the intrinsic curvature using $\nabla_X Y = \bar\nabla_X Y + B(X, Y)$:
\begin{align*}
\bar\nabla_X \bar\nabla_Y W 
&= \nabla_X(\bar\nabla_Y W) - B(X, \bar\nabla_Y W) \\
...
\]
where $\bar K$ is the sectional curvature of $N$ (which for a surface is just the Gauss curvature of $\bar g$) and $K$ is the sectional curvature of $M$ along $T_pN$. In particular, when $M = \mathbb{R}^3$ (so $K \equiv 0$), the Gauss curvature $K_{\mathrm{Gauss}} = \bar K$ is intrinsic, see Remark \ref{rem:theorema-egregium}.
...
R(X, T)\, X = \nabla_X \nabla_T X - \nabla_T \nabla_X X.
\]
Taking the inner product with $T$ and using the $(0, 4)$-convention $R(A, B, C, D) = g(R(A, B) D, C)$:
\[
