...
See also \cite{tsakanikas2024dg2}, chapter 7.3.
\end{colortheo}
...
On $\mathbb{R}$: Let $X = y^2 \frac{\mathrm{d}}{\mathrm{d}y}$ and $y_0 > 0$. The integral curve $y(t)$ satisfies $\dot{y} = y^2$, which gives
\[
...
We remark that the first axiom is natural, because $\mathcal{L}_X(\omega(Y)) = (\mathcal{L}_X \omega)(Y) + \omega(\mathcal{L}_X Y)$ mimicks a sort of chain rule.
...
But
$$w(Y) = w_\beta dx^\beta \left(Y^\alpha \frac{\mathrm{d}}{\mathrm{d}x^\alpha}\right)=w_\alpha Y^\alpha.$$
...
If $T \in T^{k,l}(M)$, and $X = \frac{\partial}{\partial x^i}$, in local coordinates then $$(\mathcal{L}_X T)^{i_1 \ldots i_k}_{j_1 \ldots j_l} = \frac{\partial T^{i_1 \ldots i_k}_{j_1 \ldots j_l}}{\partial x^i}.$$
...
We will not show the converse in full generality, but we proved a related statementin exercise 5.1. There we show that if a metric is independent of a coordinate $x^k$, then the flow $F_t$ of $\frac{\partial}{\partial x^k}$ is an isometry, $F^*_t(g|_{F_t}) = g$. We use this several times, for example in exercise 7.2b.
...
\]
One can extract the $k^{th}$ component as follows:
\[
\Gamma_{ij}^k = \mathrm{d}x^k \left( \nabla_{\frac{\partial}{\partial x^i}} \frac{\partial}{\partial x^j} \right),
\]
and interpret this as the $k^{th}$ component
$$\Gamma_{ij}^k=\left(\nabla_{\frac{\partial}{\partial x^i}} \frac{\partial}{\partial x^j}\right)^k$$
\end{colordef}

Note: $\Gamma_{ij}^k$ are not components of a tensor (see exercise 4.1a). If they would, having zero Cristoffel symbols in one coordinate system (say cartesian) would imply they vanish in all coordinate systems, which is not the case.\\
