...
\begin{colortheo}{Lie Derivative of Functions and Vector Fields}{lie_derivative_functions_vector_fields}
...
% \todo[inline]{ added this section which explains how to compute lie derivatives for any tensor }
In fact we never need a general coordinate formula. Locally, any tensor is a $C^\infty(M)$-linear combination of tensor products of the coordinate fields $\partial_i$ and $\mathrm{d}x^j$, so the Leibniz rule reduces \emph{every} computation of a Lie derivative to the action of $\mathcal{L}_X$ on these three building blocks.

% \begin{colorlem}{Computing $\mathcal{L}_X$ on any tensor}{lie_derivative_recipe}
For $X = X^m \partial_m$, the Lie derivative $\mathcal{L}_X$ is determined by $\mathbb{R}$-linearity and the Leibniz rule together with its action on functions and on the coordinate basis fields:
\begin{equation}
\label{eq:lie_derivative_recipe}
\mathcal{L}_X f = X^m\,\partial_m f, \qquad
\mathcal{L}_X \partial_i = -(\partial_i X^m)\,\partial_m, \qquad
\mathcal{L}_X \mathrm{d}x^j = (\partial_i X^j)\,\mathrm{d}x^i .
\end{equation}
% \end{colorlem}

% \begin{proof}
The first two identities follow from Theorem \ref{thm:lie_derivative_functions_vector_fields}; the third follows from the covector axiom of Lemma \ref{lem:lie_derivative_extension_tensor}: applying the LHS to a vector field $\partial_i$ gives $(\mathcal{L}_X \mathrm{d}x^j)(\partial_i) = X(\delta^j_i) - \mathrm{d}x^j([X,\partial_i]) =...= \partial_i X^j$, and the latter is exactly what we get if we evaluate the RHS on $\partial_i$.
% \end{proof}

To differentiate an arbitrary tensor, expand it in the coordinate basis and apply the Leibniz rule to each factor.



\begin{colexample}{Lie derivative of the metric}{lie_derivative_metric}
    For instance, for the metric $g = g_{ij}\,\mathrm{d}x^i \otimes \mathrm{d}x^j$,
    \begin{align*}
    \mathcal{L}_X g
    &= (\mathcal{L}_X g_{ij})\,\mathrm{d}x^i\otimes \mathrm{d}x^j
     + g_{ij}\,(\mathcal{L}_X \mathrm{d}x^i)\otimes \mathrm{d}x^j
     + g_{ij}\,\mathrm{d}x^i\otimes(\mathcal{L}_X \mathrm{d}x^j) \\
    &= \big(X^m\,\partial_m g_{ij} + g_{mj}\,\partial_i X^m + g_{im}\,\partial_j X^m\big)\,\mathrm{d}x^i\otimes \mathrm{d}x^j ,
    \end{align*}
    where in step 1 we used the Leibniz rule and in step 2 we used the first and third identity in equation \eqref{eq:lie_derivative_recipe}.
\end{colexample}


In particular, $X$ is a Killing field (Definition~\ref{def:killing}) precisely when the bracketed expression vanishes.
...
On $\mathbb{R}^n$ one can verify that the \emph{Euclidean connection} defined via the standard directional derivative $D$
\begin{equation}
...
Using the Leibnitz rule from Definition \ref{def:connection}, we can easily see that the covariant derivative of a vector field $Y$ in the direction of $X$ is given by
\begin{equation}
    \label{eq:covariant_derivative_coordinates}
    (\nabla_X Y)^k = X^i \frac{\partial Y^k}{\partial x^i} + \Gamma_{ij}^k X^i Y^j.
\end{equation}

We will now investigate how the connection acts on general tensor fields and provide coordinate formulas.

\begin{colorlem}{Extension of Connections to Tensor Fields}{connection_extension_tensor}
...
As we show in exercise 4.2, the coordinate formula for the connection of a general tensor $T$ in the direction $X=\frac{\partial}{\partial x^a}$ is
\begin{equation}
    \label{eq:covariant_derivative_tensor_coordinates}
...
% \todo[inline]{ added section about connection of arbitrary tensors }


For a function $f \in C^\infty(M)$, viewed as a $(0,0)$-tensor, the connection acts as the directional derivative, $\nabla_X f = X(f)$. This is forced by comparing two expansions of $\nabla_X(fY)$: the Leibniz rule of Definition~\ref{def:connection} gives $\nabla_X(fY) = X(f)\,Y + f\,\nabla_X Y$, while viewing $fY = f \otimes Y$ and applying the tensor Leibniz rule from Lemma \ref{lem:connection_extension_tensor} gives $\nabla_X(fY) = (\nabla_X f)\,Y + f\,\nabla_X Y$. The common term $f\,\nabla_X Y$ cancels, leaving $(\nabla_X f)\,Y = X(f)\,Y$ for all $Y$, hence $\nabla_X f = X(f)$. Writing $X = X^i \frac{\partial}{\partial x^i}$, the coordinate expression is therefore
\begin{equation}
    \label{eq:covariant_derivative_function_coordinates}
...
Similarly, the action of $\nabla$ on a basis $1$-form $\mathrm{d}x^j$ is determined once we know it on functions and on the basis vector fields. Since $\mathrm{d}x^j(\partial_i) = \delta^j_i$ is a constant function (which vanishes under the connection, see \eqref{eq:covariant_derivative_function_coordinates}) and the pairing $\omega(Y)$ is a contraction of $\omega \otimes Y$, the contraction compatibility of Lemma~\ref{lem:connection_extension_tensor} gives
\begin{align*}
    0
    &= \nabla_{\partial_a}\big(\delta^j_i\big) \\
    &= \nabla_{\partial_a}\big(\mathrm{d}x^j(\partial_i)\big) \\
    &= \nabla_{\partial_a}\big(\operatorname{tr}(\mathrm{d}x^j \otimes \partial_i)\big) \\
    &= \operatorname{tr}\big(\nabla_{\partial_a}(\mathrm{d}x^j \otimes \partial_i)\big) \\
    &= \operatorname{tr}\big((\nabla_{\partial_a}\mathrm{d}x^j) \otimes \partial_i + \mathrm{d}x^j \otimes (\nabla_{\partial_a}\partial_i)\big) \\
    &= \operatorname{tr}\big((\nabla_{\partial_a}\mathrm{d}x^j) \otimes \partial_i\big) + \operatorname{tr}\big(\mathrm{d}x^j \otimes (\nabla_{\partial_a}\partial_i)\big) \\
    &= (\nabla_{\partial_a}\mathrm{d}x^j)(\partial_i) + \mathrm{d}x^j(\nabla_{\partial_a}\partial_i)  \\
    &= (\nabla_{\partial_a}\mathrm{d}x^j)(\partial_i) + \Gamma^j_{ai}.
\end{align*}

In the very last step we have used Definition \ref{def:christoffel} to find $\nabla_{\partial_a}\partial_i = \Gamma^k_{ai}\,\partial_k$, and then $\mathrm{d}x^j(\Gamma^k_{ai}\,\partial_k) = \Gamma^k_{ai}\,\mathrm{d}x^j(\partial_k) = \Gamma^k_{ai}\,\delta^j_k = \Gamma^j_{ai}$. So we have shown
\[
(\nabla_{\partial_a}\mathrm{d}x^j)(\partial_i) = -\Gamma^j_{ai}.
\]

As a $1$-form is determined by its values on the basis fields, this means
\begin{equation}
    \label{eq:covariant_derivative_oneform_coordinates}
    \nabla_{\partial_a}\mathrm{d}x^j = -\Gamma^j_{ak}\,\mathrm{d}x^k.
\end{equation}
...
To compute the connection of a general tensor wrt. a general vector field $X$ in practice, i.e. in local coordinates, we have two options:
\begin{enumerate}[itemsep=0pt, topsep=0pt]
    \item 
    Unlike the Lie derivative, $\nabla$ is $C^\infty(M)$-linear in its direction argument, $\nabla_{fX} = f\,\nabla_X$, so $\nabla_X T = X^a\,\nabla_{\partial_a} T$ and we can use \eqref{eq:covariant_derivative_tensor_coordinates}.
    \item 
    Alternatively, to avoid having to memorise formula \eqref{eq:covariant_derivative_tensor_coordinates}, we can proceed similarly as for the Lie derivative (see \eqref{eq:lie_derivative_recipe} and Example \ref{ex:lie_derivative_metric}) and use the Leibniz rule to reduce to the coordinate basis and then apply identites \eqref{eq:covariant_derivative_function_coordinates}, \eqref{eq:covariant_derivative_coordinates}, and \eqref{eq:covariant_derivative_oneform_coordinates}, namely
    \begin{equation}
        \label{eq:connection_recipe}
        \nabla_{\partial_a} f = \partial_a f, \qquad
        \nabla_{\partial_a}\partial_i = \Gamma^k_{ai}\,\partial_k, \qquad
        \nabla_{\partial_a}\mathrm{d}x^j = -\Gamma^j_{ak}\,\mathrm{d}x^k,
    \end{equation}
    together with the $C^\infty(M)$-linearity $\nabla_X T = X^a\,\nabla_{\partial_a} T$. \\
\end{enumerate}

On a manifold, there are possibly infinitely many connections. So, this recipe is as far as we can go with a general connection: the Christoffel symbols are \emph{not} determined by the connection axioms, but are precisely the data specifying the connection (different connections have different $\Gamma^k_{ij}$), so the recipe reduces every computation to them without computing them. For the canonical connection of a Riemannian manifold, the Levi-Civita connection, the Christoffel symbols are instead given explicitly in terms of the metric; see Theorem~\ref{thm:levi_civita} and formula~\eqref{eq:christoffel_levi_civita} in the next lecture.\\



\begin{colexample}{Covariant derivative of the metric}{covariant_derivative_metric}
    Using option 2 and the identities \eqref{eq:connection_recipe}, we compute the covariant derivative of a general metric $g = g_{ij}\,\mathrm{d}x^i \otimes \mathrm{d}x^j$ in the direction of a general vector field $X = X^a \partial_a$, exactly as we did for the Lie derivative in Example~\ref{ex:lie_derivative_metric}. The Leibniz rule gives
    \begin{align*}
    \nabla_{X} g
    &= (\nabla_{X} g_{ij})\,\mathrm{d}x^i\otimes \mathrm{d}x^j
     + g_{ij}\,(\nabla_{X} \mathrm{d}x^i)\otimes \mathrm{d}x^j
     + g_{ij}\,\mathrm{d}x^i\otimes(\nabla_{X} \mathrm{d}x^j) \\
    &= X^a(\nabla_{\partial_a} g_{ij})\,\mathrm{d}x^i\otimes \mathrm{d}x^j
     + g_{ij}\,X^a(\nabla_{\partial_a} \mathrm{d}x^i)\otimes \mathrm{d}x^j
     + g_{ij}\,X^a\,\mathrm{d}x^i\otimes(\nabla_{\partial_a} \mathrm{d}x^j) \\
    &= X^a(\partial_a g_{ij})\,\mathrm{d}x^i\otimes \mathrm{d}x^j
     - g_{ij}\,X^a\Gamma^i_{ak}\,\mathrm{d}x^k\otimes \mathrm{d}x^j
     - g_{ij}\,X^a\Gamma^j_{ak}\,\mathrm{d}x^i\otimes \mathrm{d}x^k \\
    &= X^a\big(\partial_a g_{ij} - \Gamma^m_{ai}\,g_{mj} - \Gamma^m_{aj}\,g_{im}\big)\,\mathrm{d}x^i\otimes \mathrm{d}x^j ,
    \end{align*}
    where in the second step we used the $C^\infty(M)$-linearity $\nabla_X = X^a\,\nabla_{\partial_a}$, in the third the building blocks \eqref{eq:covariant_derivative_function_coordinates} and \eqref{eq:covariant_derivative_oneform_coordinates}, and in the fourth we relabelled the summation indices. And of course the components are $(\nabla_{X} g)_{ij} = X^a\big(\partial_a g_{ij} - \Gamma^m_{ai}\,g_{mj} - \Gamma^m_{aj}\,g_{im}\big)$.
\end{colexample}

% \begin{colexample}{}{}

% \todo[inline]{ clean up, explain and add example that explains how to use the recipe above }

% For a (0,1)-tensor, i.e. a 1-form, $\omega = \omega_i \mathrm{d}x^i$:
% \[
% (\nabla_X \omega)_k = X^i \frac{\partial \omega_k}{\partial x^i} - \Gamma_{ik}^{\alpha} X^i \omega_\alpha
% \]

% For $(1,1)$ tensor fields, $T=T^i_j \partial_i \otimes \mathrm{d}x^j$:
...
\begin{colremark}[Lie derivative vs.\ covariant derivative]
Both $\mathcal{L}_X$ and $\nabla_X$ turn a direction field $X$ and a tensor into a tensor of the same type, but they differ fundamentally. Comparing the two computations for the metric (Examples~\ref{ex:lie_derivative_metric} and~\ref{ex:covariant_derivative_metric}),
\begin{align*}
(\mathcal{L}_X g)_{ij} &= X^m\,\partial_m g_{ij} + g_{mj}\,\partial_i X^m + g_{im}\,\partial_j X^m, \\
(\nabla_X g)_{ij} &= X^a\big(\partial_a g_{ij} - \Gamma^m_{ai}\,g_{mj} - \Gamma^m_{aj}\,g_{im}\big),
\end{align*}
Some differences stand out:
\begin{enumerate}[itemsep=0pt, topsep=0pt]
    \item \emph{Dependence on $X$.} In $\nabla_X g$ the field $X$ enters only through the pointwise factor $X^a$, so $(\nabla_X g)|_p$ depends only on $X|_p$ (indeed $\nabla$ is $C^\infty(M)$-linear in $X$). In $\mathcal{L}_X g$ the derivatives $\partial_i X^m$ appear, so $(\mathcal{L}_X g)|_p$ depends on $X$ in a whole neighbourhood of $p$ ($\mathcal{L}$ is only $\mathbb{R}$-linear in $X$). Only $\nabla_X$ is a genuine generalisation of the directional derivative.
    \item \emph{Required data.} $\mathcal{L}_X$ uses only the smooth structure (via the flow of $X$), whereas $\nabla_X$ requires a choice of connection, i.e.\ the Christoffel symbols $\Gamma^k_{ij}$.
    \item Further, on vector fields the two are related directly: $\mathcal{L}_X Y = [X,Y] = \nabla_X Y - \nabla_Y X$ whenever $\nabla$ is \emph{torsion-free}, see Definition~\ref{def:torsion} below.
\end{enumerate}
\end{colremark}
...
\begin{colordef}{Torsion of a Connection}{torsion}
