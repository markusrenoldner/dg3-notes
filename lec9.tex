...
\[
R_{2} : \Lambda^2 T_p M \times \Lambda^2 T_p M \to \mathbb{R}, \qquad R_{2}(X \wedge Y,\, Z \wedge W) := R(X, Y, Z, W),
\]
and the pairwise symmetry (item 3) makes $R_{2}$ a \emph{symmetric} bilinear form on $\Lambda^2 T_p M$. In this picture the sectional curvature reads
\[
K(\Pi) = \frac{R_{2}(X \wedge Y,\, X \wedge Y)}{\|X \wedge Y\|^2},
\]
