...
This is a special case of the following general algebraic fact. Let $V$ be a vector space and $B: V \times V \to \mathbb{R}$ a bilinear, alternating form (i.e.\ $B(X, Y) = -B(Y, X)$ and $B(X, X) = 0$ for any $X$). Then $B$ \emph{factors through} $\wedge: V\times V\to \Lambda^2 V$: there is a unique linear map $\widetilde B: \Lambda^2 V \to \mathbb{R}$ such that
\[
