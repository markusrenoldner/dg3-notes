...
\begin{tikzpicture}[scale=5, >=stealth, line join=round]
  ...
  % Z before transport
  \draw[->, Leman, very thick] (0,0) -- (0.30, 0.55) node[pos=1, above] {$Z$};
...
% \begin{colremark}[$R$ depends only on the bivector $X \wedge Y$]{}
%     \label{rem:R-map-on-bivectors111}
% $R(X, Y)$ is bilinear and antisymmetric in its first two arguments, $R(X, Y) = -R(Y, X)$. Together these mean $R(X, Y)$ does not depend on $X$ and $Y$ separately, but only on the \emph{oriented parallelogram} they span: if $(X', Y')$ spans the same plane with the same orientation and area, then $R(X', Y') = R(X, Y)$.

% We give this parallelogram a name: the \emph{bivector}
% \[
% X \wedge Y \in \Lambda^2 T_p M,
% \]
% the oriented plane element with $X \wedge Y = -\,Y \wedge X$, and $X \wedge Y = 0$ exactly when $X, Y$ are parallel. The sign flip $X \wedge Y = -Y \wedge X$ is precisely the antisymmetry $R(X, Y) = -R(Y, X)$, so we may regard $R$ as a function of the single input $X \wedge Y$. (This is general: any bilinear, antisymmetric quantity depends only on the bivector.)
% \end{colremark}
...
This is a special case of the following general algebraic fact. Let $V,W$ be vector spaces and $B: V \times V \to W$ a bilinear, alternating map (i.e.\ $B(X, Y) = -B(Y, X)$ and $B(X, X) = 0$ for any $X$). Then $B$ \emph{factors through} $\wedge: V\times V\to \Lambda^2 V$: there is a unique linear map $B_\wedge: \Lambda^2 V \to W$ such that
\[
B(X, Y) = B_\wedge(X \wedge Y) \qquad \text{for all } X, Y \in V.
\]
One can check this by defining $B_\wedge(X \wedge Y) := B(X, Y)$ on simple bivectors and extending by linearity, then verifying well-definedness (the relations defining $\Lambda^2 V$ are respected) and uniqueness (simple bivectors $X \wedge Y$ span $\Lambda^2 V$).
\[
\begin{tikzcd}[row sep=large, column sep=large]
V \times V \arrow[r, "\wedge"] \arrow[dr, "B"'] & \Lambda^2 V \arrow[d, dashed, "B_\wedge"] \\
& W
\end{tikzcd}
\]
\end{colremark}
...
The other direction ($R = 0 \implies$ locally flat) is more delicate. It is proved in Exercise~10.4(c) for the shape operator of geodesic spheres.
\end{proof}
...
% Recall the bivector $X \wedge Y \in \Lambda^2 T_p M$ from Remark~\ref{rem:R-map-on-bivectors111}; the space $\Lambda^2 T_p M$ of all bivectors (or 2-vectors) is built from vectors by the same wedge construction used for differential forms, just applied to vectors instead of covectors. It carries a natural inner product, determined by
% \[
% \langle X \wedge Y,\, Z \wedge W\rangle = \langle X, Z\rangle \langle Y, W\rangle - \langle X, W\rangle \langle Y, Z\rangle.
% \]
% In particular $\|X \wedge Y\|^2 = \|X\|^2 \|Y\|^2 - \langle X, Y\rangle^2$, which is exactly the squared area of the parallelogram spanned by $X, Y$ --- the denominator of the sectional curvature.

% Feeding two bivectors into the $(0,4)$ tensor,
% \[
% \widetilde R : \Lambda^2 T_p M \times \Lambda^2 T_p M \to \mathbb{R}, \qquad \widetilde R(X \wedge Y,\, Z \wedge W) := R(X, Y, Z, W),
% \]
% is well-defined on bivectors by the antisymmetries of $R$ (Theorem~\ref{thm:symmetries-riemann-curvature-tensor}, items 1 and 2), and the pairwise symmetry (item 3) makes $\widetilde R$ a \emph{symmetric} bilinear form on $\Lambda^2 T_p M$. With this notation the sectional curvature reads
% \[
% K(\Pi) = \frac{\widetilde R(X \wedge Y,\, X \wedge Y)}{\|X \wedge Y\|^2},
% \]
% a ratio (like a Rayleigh quotient) depending only on the bivector $X \wedge Y$ up to scaling, equivalently only on the plane $\Pi = \mathrm{span}(X, Y)$.
% \end{colremark}

\begin{colremark}[$R$ as a quadratic form on bivectors]{}
\label{rem:R-map-on-bivectors222}

% Recall Section \ref{sec:notation-tensors} and Remark \ref{rem:R-map-on-bivectors111}: $R$ can be viewed as a map $\widetilde R$ on bivectors $X \wedge Y \in \Lambda^2 T_p M$. The induced inner product on $\Lambda^2 T_p M$ is
Recall Section~\ref{sec:notation-tensors} and Remark~\ref{rem:R-map-on-bivectors111}, where the bivectors $X \wedge Y \in \Lambda^2 T_p M$ were introduced and $R$ was viewed as the operator-valued map $R_\wedge$ acting on a \emph{single} bivector. The map $\widehat R_\wedge$ defined below is different: it takes \emph{two} bivectors and returns a scalar in $\mathbb{R}$. $\widehat R_\wedge$ is the $(0,4)$ analogue of $R_\wedge$, exactly as $\widehat R$ is of $R$. (For the tensors $R$ and $\widehat R$ we follow the convention to drop the hat symbol as remarked earlier).

First, note that the natural inner product on $\Lambda^2 T_p M$ is defined by
\[
\langle X \wedge Y,\, Z \wedge W\rangle = \langle X, Z\rangle \langle Y, W\rangle - \langle X, W\rangle \langle Y, Z\rangle,
\]
so in particular $\|X \wedge Y\|^2 = \|X\|^2 \|Y\|^2 - \langle X, Y\rangle^2$, matching the denominator of $K$.
...
\[
\widehat R_\wedge : \Lambda^2 T_p M \times \Lambda^2 T_p M \to \mathbb{R}, \qquad \widehat R_\wedge(X \wedge Y,\, Z \wedge W) := R(X, Y, Z, W),
\]
and the pairwise symmetry (item 3) makes $\widehat R_\wedge$ a \emph{symmetric} bilinear form on $\Lambda^2 T_p M$. In this picture the sectional curvature reads
\[
K(\Pi) = \frac{\widehat R_\wedge(X \wedge Y,\, X \wedge Y)}{\|X \wedge Y\|^2},
\]
