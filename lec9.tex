...
\begin{colordef}{The (0,4) Riemann curvature tensor}{}
...
Since $R$ is antisymmetric in its first two arguments, we can view it as a map on bivectoors:
\[
R(X, Y) = \widetilde R(X \wedge Y),
\]
where $X \wedge Y \in \Lambda^2 T_p M$. The antisymmetry $X \wedge Y = -Y \wedge X$ in $\Lambda^2 T_p M$ then automatically encodes the antisymmetry $R(X, Y) = -R(Y, X)$.

This is a special case of the following general algebraic fact. Let $V$ be a vector space and $B: V \times V \to \mathbb{R}$ a bilinear, alternating form (i.e.\ $B(X, Y) = -B(Y, X)$). Then $B$ \emph{factors through} the exterior product: there is a unique linear map $\widetilde B: \Lambda^2 V \to \mathbb{R}$ such that
\[
...
By the antisymmetries of $R$ (Theorem~\ref{thm:symmetries-riemann-curvature-tensor}, items 1 and 2), the curvature tensor descends to a bilinear map
\[
...
$$\nabla_X Y = \pi^T(\nabla_X Y) + \pi^\perp(\nabla_X Y),$$
where the first term is the induced connection on $N$, and the second one the \emph{second fundamental form}.
